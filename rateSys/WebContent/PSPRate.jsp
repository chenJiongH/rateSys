		...
		function showProcess(pData) {
			...
			while(data.process.indexOf('-') != -1) {
				...
				if(rank == "校") {
					...
					$("div[class=ratemessage]").append('<label style="margin-right: 53px;">县（市、区）级</label>');
				} else if(rank == "市") {
					$("div[class=ratemessage]").append('<label style="margin-right: 92px;">设区市级</label>');
				} else if(rank == "省"){
			...
			$(function() {
				...
				var bottomBarTop = (top + 722) + "px";
			 	$(".bottom-bar").css({
						"top": bottomBarTop
				});
			})
			    //查找页面待显示内容，即当前校级管理员指标内容
				$.post("PSPRateServlet/findPr", function(data) {
					mana = data[data.length-1];
					console.log(data); 
					...
					$(".unit").text(data[data.length-1].sporganization); 
					//显示所有已开启并所有学校完成自评的项目
					projects = data;
					isOnSpot = projects[0].IsOnSpot;
					// 省级专家删除抽检专家评分栏 
 					if(isOnSpot == 0) {
						$("#row1 th:eq(11)").css("display", "none"); 
						$("#row1 th:eq(12)").css("display", "none"); 
						$("#row2 th:eq(13)").css("display", "none");
						$("#row2 th:eq(14)").css("display", "none");
						$("table").css("width", "1826px");
					} else if(isOnSpot == 1) { 
 						$("#row1 th:eq(11)").css("display", "table-cell"); 
						$("#row1 th:eq(12)").css("display", "table-cell"); 
						$("#row2 th:eq(13)").css("display", "table-cell");
						$("#row2 th:eq(14)").css("display", "table-cell");
 						$("table").css("width", "2251px");
					} 
					var options = "";
					...
					$.post("PSPRateServlet/findCByPid", {'pid' : $("#Pr").val(), 'isOnSpot' : isOnSpot}, function(data) {
						console.log(data);
						if(data.message != undefined) {
							$("#message").html("<img src='images/warn.png' class='messageWarn'>" + data.message);
							messageDisplay();
						...
						pageBean = data;
						if(curpage == "") curpage = data.sch[0].sid;
// 						构造所有的sidcid键值对，用空间换取时间、减少提交时判断数
						var scoreName = "proscore", explainName = "proexplain";
						if(isOnSpot == 1) {
							scoreName = "onSpotScore";
							explainName = "onSpotExplain";
						}
						for(var i = 0; i < data.c.length; i++) {
							// 如果分数为空，要填写分数
							if(data.c[i][scoreName] === "" || data.c[i][scoreName] === null)
							...
							// 如果分数不为空，要根据分数和阈值、是否说明，来填写说明项。 
								...
								if(data.c[i][scoreName] < data.c[i].threshhold && (data.c[i].isexplain == 'TRUE' || data.c[i].isexplain== 'true') && data.c[i][explainName] === "")
				...
				$("#Pr").change(function() {
					...
					for(var i = 0; i < projects.length - 1; i++) {
						if(projects[i].pid == $("#Pr").val()) {
							isOnSpot = projects[i].IsOnSpot;
							console.log(projects[i].pid + " : " + projects[i].commState);
...
// 						构造所有的sidcid键值对，用空间换取时间、减少提交时判断数
						...
						dataShow(data);
						if(data.sch.length == 0) {
							$("#message").html("<img src='images/warn.png' class='messageWarn'>" + "该项目暂时未添加学校");
							messageDisplay();
							setTimeout(hid, 3000);
						}
					}, 'json') 
			...
			function setReadonly() {
				...
				$("input[value='查阅总评']").removeAttr("onclick");
			...
			function dataShow(data) {
				...
				for(var i = 0; i < data.c.length && start != 2; i ++) {
					if(curpage == data.c[i].sid) {
						...
						if(aname != data.c[i].aname)
							...
							trs += "<td class='t1' style='left: " + nowLeft + ";' title='"+data.c[i].aname+"'>" + data.c[i].aname + "</td>";
						else trs += "<td class='t1' style='left: " + nowLeft + ";'></td>"; 
						if(bname != data.c[i].bname)
							bname = data.c[i].bname,
							trs += "<td class='t2' style='left: " + nowLeft + ";' title='" + data.c[i].bname+"'>" + data.c[i].bname + "</td>";
						else trs += "<td class='t2' style='left: " + nowLeft + ";' ></td>";
						trs += "<td class='t3' style='left: " + nowLeft + ";' title='" +data.c[i].cname +"'>" + data.c[i].cname + "</td>";
						trs += "<td class='t4' style='left: " + nowLeft + ";'>" + data.c[i].score + "</td>";
						trs += "<td class='t5' style='left: " + nowLeft + ";'>" + data.c[i].schoolscore + "</td>";
						trs += "<td class='t6' style='left: " + nowLeft + ";'><input type='button' value='查看' name='" + data.c[i].description + "' onclick='lookDescri(this)'></td>";
						...
						if(data.c[i].cityScore == null) data.c[i].cityScore = "";
						if(data.c[i].cityExplain == null) data.c[i].cityExplain = "";
						trs += "<td class='t7'><input type='text' readonly='readonly' value='" + data.c[i].districtScore + "'></td>";
						trs += "<td class='t8' title='"+data.c[i].districtExplain+"'><input readonly='readonly' type='text' value='" + data.c[i].districtExplain + "'></td>";
						trs += "<td class='t7'><input type='text' readonly='readonly' value='" + data.c[i].cityScore + "'></td>";
						trs += "<td class='t8' title='"+data.c[i].cityExplain+"'><input readonly='readonly' type='text' value='" + data.c[i].cityExplain + "'></td>";
						var scoreName = "proscore", explainName = "proexplain";
						if(isOnSpot == 1) {
							if(data.c[i].proscore == null) data.c[i].proscore = "";
							if(data.c[i].proexplain == null) data.c[i].proexplain = "";
							trs += "<td class='t7'><input type='text' readonly='readonly' value='" + data.c[i].proscore + "'></td>";
							trs += "<td class='t8' title='"+data.c[i].proexplain+"'><input readonly='readonly' type='text' value='" + data.c[i].proexplain + "'></td>";
							scoreName = "onSpotScore";
							explainName = "onSpotExplain";
						}
						console.log(scoreName + " : "+ explainName);
						if(data.c[i].isexplain == 'TRUE' || data.c[i].isexplain== 'true') {/*  是否需要说明，无需说明则设置阈值为负无穷大  */
							sidcidExplain[data.c[i].sid + "" + data.c[i].cid] = true;
							if((data.c[i][scoreName] != null && data.c[i][scoreName] !== "") || formData[data.c[i].sid + "" + data.c[i].cid] != undefined) {
								delete sidcidScore[data.c[i].sid + "" + data.c[i].cid];
								if(formData[data.c[i].sid + "" + data.c[i].cid] == undefined) {
									// 清除说明标记 
									if(data.c[i].cityScore >= data.c[i].threshhold) 
										delete sidcidExplain[data.c[i].sid + "" + data.c[i].cid];
									formData[data.c[i].sid + "" + data.c[i].cid] = data.c[i][scoreName];
									// id=sid+cid name=阈值 class=分段值  sidcidExplain
									trs += "<td class='t7' title='分段值:" + data.c[i].segscore +"'><input segscore='" + data.c[i].segscore +"' type='text' id='" + data.c[i].sid + "" + data.c[i].cid +"' name='" + data.c[i].threshhold +"'  class='TRUE' onblur='grade(this)' value='" + data.c[i][scoreName] + "'></td>";
								} else {		
							...
							if((data.c[i][scoreName] != null && data.c[i][scoreName] !== "") || formData[data.c[i].sid + "" + data.c[i].cid] != undefined) {
								...
								if(formData[data.c[i].sid + "" + data.c[i].cid] == undefined) {
									formData[data.c[i].sid + "" + data.c[i].cid] = data.c[i][scoreName];
									trs += "<td class='t7' title='分段值:" + data.c[i].segscore +"'><input segscore='" + data.c[i].segscore +"' type='text' id='" + data.c[i].sid + "" + data.c[i].cid +"' name=''  onblur='grade(this)' value='" + data.c[i][scoreName] + "'></td>";
								} else {
						...
						if(data.c[i][explainName] != null && data.c[i][explainName] != "" || formData["0" + data.c[i].sid + data.c[i].cid] != undefined) {
							delete sidcidExplain[data.c[i].sid + "" + data.c[i].cid];
							if(formData["0" + data.c[i].sid + data.c[i].cid] == undefined) {
								formData["0" + data.c[i].sid + data.c[i].cid] = data.c[i][explainName];
								trs += "<td class='t8' title='"+data.c[i][explainName]+"'><input maxlength='300' type='text' class='0" + data.c[i].sid + "" + data.c[i].cid + "' onblur='explain(this)' value='" + data.c[i][explainName] + "'></td>";
							} else {
				...
				// 刷新页面显示到自己的评分位置
				$(".bottom-bar span").css({"left": "351px"});
				var nowLeft = "";
				if(isOnSpot == 1)
					nowLeft = "1312px";
				else 
					nowLeft = "869px";
				$("table").css({"left": "-" + nowLeft})
				var trs = $("table").find("tr:eq(0)");
				trs.each(function() {
					for(var i = 0 ; i <= 4; i++) 
						$(this).children().eq(i).css({"left": nowLeft});
				})
			    // 控制第2行前7个元素不动 
			    ...
			    if(readonly == true) {
					...
					$(":input[value='查阅总评']").attr("onclick", "lookDescri(this)");	
...
// 				获取小数位数, 将有可能为小数的分值段转化为整数，再利用取余运算符
				...
				// parseFloat((Math.pow(10, num) * $(e).val())).toFixed(5);
				// .toFixed(5) 是对前面5个小数进行四舍五入。不然如1.1*100 不是 110 而是 110.0000000000001。会出现计算误差
				if(parseFloat((Math.pow(10, num) * $(e).val())).toFixed(5) % (Math.pow(10, num) * $(e).attr("segscore")) != 0) { 
			...
			function explain(e) {
				...
				if($(e).val() != "") {
					...
					var sroreInput;
					if(isOnSpot == 1)
						scoreInput = $($(e).parent().parent().find("td:eq(13)").children("input").get(0));
					else if(isOnSpot == 0)
						scoreInput = $($(e).parent().parent().find("td:eq(11)").children("input").get(0));
			...
			function rate() {
		      ...
		      zdconfirm('系统确认框',"提交后，评分信息将不可修改、无法回退。请确定是否提交？",function(r){ //项目删除确定框 
	    	     if(r) {
					formData.pid = $("#Pr").val();
					formData.isOnSpot = isOnSpot;
					$.post("PSPRateServlet/rate", {'c' : JSON.stringify(formData)}, function(data) {
	...
	<body>
	<div class="wrapper" style="width:100%; height:88rem; margin:0 auto;">
		<div class="header">
...
<!-- 				用于下载总评或附件 -->
				...
				<form id='form1' method='post' action='PSPRateServlet/rate' enctype="multipart/form-data">
					<span class="breakH" style='top: 12.5rem;margin-left: -93px;'></span>
					<span class="breakV" style='top: 9.3rem;margin-left: -93px;'></span>
					...
					<div id='editorBox' style='height: 76rem; margin-bottom: 2.9rem; z-index: 99999; width: 93rem; background-color: rgb(249, 249, 249); display: none; position: absolute; top: -86px; left: -25%;'>
			...
			<!-- 			编辑框 -->
					...
					<span style="position:absolute;right: 94%;top: 55%;width: 103px;margin-top:-0.6rem;">项目名称：</span>
					<select id='Pr' style="position:absolute;right: 66%;outline:none;"></select>
				...
				<div class="pagediv">
					...
					<table>
  						<tr id='row1'>
							<th colspan='3' style='background-color: rgb(236, 239, 253);z-index:3000;width: 228px;'></th>
							<th class='t5'  style='background-color: rgb(236, 239, 253);z-index:3000;'></td>
							<th class='t5'  style='background-color: rgb(236, 239, 253);z-index:3000;'></td>
							<th class='t6' style='background-color: rgb(236, 239, 253);z-index:3000;' ></td>
							...
							<th class='t8' style='text-indent:-18.4rem;background-color: rgb(236, 239, 253);z-index:2000;' >县级专家</th>
							<th class='t7 widChan'  style='background-color: rgb(236, 239, 253);z-index:2000;'></th>
							<th class='t8' style='text-indent:-18.4rem;background-color: rgb(236, 239, 253);z-index:2000;' >市级专家</th>
							<th class='t7 widChan'  style='background-color: rgb(236, 239, 253);z-index:2000;'></th>
							<th class='t8' style='text-indent:-18.4rem;background-color: rgb(236, 239, 253);z-index:2000;' >省级专家</th>
							<th class='t7 widChan'  style='background-color: rgb(236, 239, 253);z-index:2000;'></th>
							<th class='t8' style='text-indent:-18.4rem;background-color: rgb(236, 239, 253);z-index:2000;' >抽检专家</th>
						</tr>
						<tr id='row2'>
					...
					<!-- 自定义滚动条 -->
					<div class="bottom-bar" style="left: 506px;top: 722px;">
		                <span></span>
		            </div>
					</div>
			</div>
...
(function($) {
    ...
    var GenerateCss = function () {
        ...
        $("#mb_msg").css({ padding: '2rem 0px 2rem 10px', lineHeight: '2rem',
