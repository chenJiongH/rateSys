					...
					for(var i = 0; i < pageBean.ps.length; i++) {
						if(pid == pageBean.ps[i].pid) {
							if(pageBean.ps[i].pprocess == null || pageBean.ps[i].pprocess.indexOf("抽查") == -1) {
								...
								$("#spotMess").css("display", "inline-block");
...
// 				</tr>
		    	 $("table").html("");
	    		 trs = "<tr><th class='t1'>项目名称</th><th class='t2'>专家组名称</th><th class='t3'>级别</th><th class='t3'>操作</th></tr>";
	    		 $("table").append(trs);
		    	 trs = "";
		    	 var tableJson = {};
	...
	// 						</tr>  
			    			...
			    			if(flag) 
				    		 ...
				    		 var isonspot = (tableJson.gs[j].isonspot == "1"?"抽查专家组":"省级专家组");
				    		 trs += "<td class='t3'>" + isonspot + "</td>";
		    	 ...
		    	 $("table tr:eq(1)").click();
					if($("table tr:eq(1)").find("td:eq(2)").text() == "抽查专家组")
						$("#onSpot").prop("checked", true);
					else $("#onSpot").prop("checked", false);
		    	 flush = true;
		    	 $("#projectName").change();
			...
			function delData(e) {
		      zdconfirm('系统确认框','确定删除该专家组吗？',function(r){ //项目删除确定框 
	    	     if(r) {					
					 //确定删除项目后，删除无指标项目 
					$.post("ExGroupsServlet/delData",{'spgid': $(e).parent().parent().find("td:eq(4)").text()}, function(data) {
			...
			function trClick(e) {
				...
				$("#gid").val($(e).find("td:eq(4)").text());
// 				console.log($(e).find("td:eq(2)").text());
				if($(e).find("td:eq(2)").text() == "抽查专家组")
					$("#onSpot").prop("checked", true);
				else $("#onSpot").prop("checked", false);
...
<!-- 					<a href="changeSubPin.jsp"><li><img src="images/left2.png">下级密码</li></a> -->
					...
					<a href="exGroups.jsp"><li class="now"><img src="images/left6-s.png">新建专家组</li></a>
...
<!-- 						<li onclick='mayNameClick(this)'>1231</li> -->
					...
					<!-- 这是实地考察的流程，关闭则取消考察流程。开启则有抽查流程 -->
					<span id='spotMess' style='position: absolute;right: 27%;top: 59%;font-size: 1.9rem;'>实地考察：</span><input class='onSpot' style="display : none;" name='onSpot' value='1' id='onSpot' type='checkbox' ><label></label>
				</form>
				<div class='tdiv'>
				<table>
					<thead>
