	...
	// 						"background-color": "white",
				...
				$.post("TaskMServlet/findCDG", function(data) {
					...
					if(data.pros.length > 0)
						options = "<option value='" + data.pros[0].pid + "' name=" + data.pros[0].Pname + ">" + data.pros[0].Pname + "</option>";
					for(var i = 1; i < data.pros.length; i++)
						options += "<option value='" + data.pros[i].pid + "' name=" + data.pros[i].Pname + ">" + data.pros[i].Pname + "</option>";
					$("#group").html(options);
...
// 					console.log(pid);
					$.post("TaskMServlet/findPageBean", {'getAllspgsch' : 'true' , 'pid':$('#group').val(), 'cid' : $('#city').val(), 'did' : $('#dist').val(), }, function(data) {
						console.log(data);
						pageBeanData = data;
						spgsch = data.spgsch;
						changePageFlag = true;
				...
				$("#dist").change(function() { //修改 ： 只获取ss（所有校） 
					$.post("TaskMServlet/findPageBean", {'pid':$('#group').val(), 'cid' : $('#city').val(), 'did' : $('#dist').val()}, function(data) {
						console.log(data);
						pageBeanData = data;
					    mulTrDrag2 = {}; //多选右表格单元行 
						dataShow();
...
// 		            e.target.style.opacity = 0.7; //被拖动时 模糊处理  
	            ...
	            document.ondrop = function (e) {
					...
					if(tableDown1 == "table1" || tableDown2 == "table1") {
						...
						//批量增加sids校组任务 
						$.post("TaskMServlet/addSpgsch", {'pid':$('#group').val(), 'cid' : $('#city').val(), 'did' : $('#dist').val(), 'sids' : JSON.stringify(sids)}, function(data) {
							if(data.message != undefined)
								return ; 
							var groupName = "";
							var pname = "";
...
//			 		            	this.style.opacity = 0.7; //被选中时 模糊处理
						...
						console.log(sids);
						//删除数据，获取新页面数据 
						$.post("TaskMServlet/delSpgsch", {'pid':$('#group').val(), 'cid' : $('#city').val(), 'did' : $('#dist').val(), 'sids' : JSON.stringify(sids)}, function(data) {
...
// 			            	this.style.opacity = 0.7; //被选中时 模糊处理
			...
			function dataShowLeft() {
		    	 $("#table1").html("");
				 var trs = "<tr style='background-color: #eceffd;'><th class='t2'>学校名称</th><th class='t3'>项目</th><th class='t4'>管理员</th></tr>";
	    		 $("#table1").append(trs);
				 var tableJson = {};
		    	 tableJson.spgsch = [];
				var gid = $("#group").val();
...
// 		            	this.style.opacity = 0.7; //被选中时 模糊处理
	...
	<div class="wrapper" style="width:190.3rem; height:88rem; margin:0 auto; position:relative;">
		...
		<div class="main">
			<div class="location">当前位置：主页&nbsp;&nbsp;>&nbsp;&nbsp;项目学校任务</div>
			<div class="left">
...
<!-- 					<a href="changeSubPin.jsp"><li><img src="images/left2.png">下级密码</li></a> -->
					...
					<a href="taskM.jsp"><li class="now"><img src="images/left7-s.png">项目学校任务</li></a>
			...
			<div class="right">
				<div class="rhead">
					项目学校任务<span>PROJECT SCHOOL TASK</span>
				</div>
				<form class="center">
