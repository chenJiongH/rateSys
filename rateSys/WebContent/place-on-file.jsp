		...
		function hid() { 
			...
			function dataShow(data) {
				...
				for(var i = 0; i < data.length ; i ++) {
					...
					if(data[i].pspScore == null) data[i].pspScore = "";
					else scoreFlag[3] = 1;
					if(data[i].pspExplain == null) data[i].pspExplain = "";
					if(data[i].pspName == null) data[i].pspName = "";
					scoreType[3] += Number(data[i].pspScore);
					trs += "<td class='t7'><input type='text' readonly='readonly' value='" + data[i].pspScore + "'></td>";
					trs += "<td class='t9'><input type='text' readonly='readonly' value='" + data[i].pspName + "'></td>";
					trs += "<td class='t8' title='"+data[i].pspExplain+"'><input readonly='readonly' type='text' value='" + data[i].pspExplain + "'></td>";
					
					if(data[i].checkScore == null) data[i].checkScore = "";
					else scoreFlag[4] = 1;
					if(data[i].checkExplain == null) data[i].checkExplain = "";
					if(data[i].checkspName == null) data[i].checkspName = "";
					scoreType[4] += Number(data[i].checkScore);
					trs += "<td class='t7'><input type='text' readonly='readonly' value='" + data[i].checkScore + "'></td>";
					trs += "<td class='t9'><input type='text' readonly='readonly' value='" + data[i].checkspName + "'></td>";
					trs += "<td class='t8' title='"+data[i].checkExplain+"'><input readonly='readonly' type='text' value='" + data[i].checkExplain + "'></td>";
			...
			function exportScore() {
				$("#message").html("<img src='images/warn.png' class='messageWarn'>正在导出所有评分");
				messageDisplay();
				...
				// 一个项目多个学校在下拉框，先请求所有该项目下所有的学校数据，然后在判断已经是最后一个学校完成时，开始导出
				$("#school").children().each(function() {
					...
					$.ajax({
						...
						success:function(data){
							...
							// 在最后一个学校数据收集成功时，才发出导出请求
							if(sname == $("#school option:last").val()) {
								exportAllSchoolList.push(location);
								$("#allData").val(JSON.stringify(exportAllSchoolList));
								$("#exportScore").submit();
								setTimeout(hid, 8000);
							}
						},
...
<!-- 					<a href="changeSubPin.jsp"><li><img src="images/left2.png">下级密码</li></a> -->
					...
					<a href="place-on-file.jsp"><li class='now'><img src="images/left9-s.png">项目评分详情</li></a>
			...
			<div class="right">
				...
				<form id='exportScore' style='display:none' action="PlaceOnFileServlet/exportScore" method='post'>
					<input type='text' id='allData' name='data' value=''>
				</form>
				<!-- 用于导出页面评分数据 -->
...
<!-- 				用于下载总评或附件 -->
				...
				<form id='form1' method='post' action='PlaceOnFileServlet/rate' enctype="multipart/form-data">
					<span class="breakH" style='top: 128px;margin-left: 27px;width: 512px;'></span>
					<span class="breakV" style='top: 99px;margin-left:-160px;'></span>
			...
			<!-- 			编辑框 -->
					...
					<div class="prType">
						<!-- 标识项目是否为总评或归档 -->
						<input id='doing' style='width: 20px;height: 15px;' type="radio" name='prType' value='exist'/><span>在评项目</span>
						<input id='down' style='width: 20px;height: 15px;' type="radio" name='prType' value='end'/><span>结束项目</span>
					...
					<div class='scoreType'>
						...
						省：<span id='proScore'>未评分</span>
						抽查：<span id='spotScore'>未评分</span>
					</div>
					<!-- 模拟项目动态下拉框 -->
					...
					<input style='position: absolute;top: 41px;left: 6.2rem;' type='text' id='prInput' list='Pr' />
					<datalist id='Pr'></datalist>
					<!-- 模拟学校动态下拉框 -->
					<!-- <select id='school' style="position:absolute;right: 35%;top: 181%;outline:none;"></select> -->
					<span style="position:absolute;right: 61%;top: 55%;top: 228%;margin-top:-5px;">学校：</span>
					<input style='position: absolute;top: 41px;left: 372px;' type='text' id='schoolInput' list='school' />
					...
					<input type="button" style='right: 2%;top: 181%;background:#4bc1d1' name="" id="" class='in3' value="导出所有评分" onclick='exportScore()'/>
				</form>
				<div class="pagediv" style='width: 0rem;'>
					<ul class="page">
					</ul>
				</div>

					<div id='tdiv' class='tdiv' style='margin-top: 14px;'>
					<table style='    width: 2011px;'>
  						<tr id='row1'>
							<th colspan='3' style='background-color: rgb(236, 239, 253);z-index:3000;width: 155px;'></th>
							...
							<th class='t7 widChan'  style='background-color: rgb(236, 239, 253);z-index:2000;'></th>
							<th class='t9' style='z-index:2000;'></th>
							<th class='t8' style='text-indent:-174px;background-color: rgb(236, 239, 253);z-index:2000;' >省级专家</th>
							
							<th class='t7 widChan'  style='background-color: rgb(236, 239, 253);z-index:2000;'></th>
							<th class='t9' style='z-index:2000;'></th>
							<th class='t8' style='text-indent:-174px;background-color: rgb(236, 239, 253);z-index:2000;' >抽检专家</th>
						</tr>
						<tr id='row2'>
					...
					<!-- 自定义滚动条 -->
					<div class="bottom-bar" style="width: 512px;left: 424px;top: 598px;">
		                <span></span>
		            </div>
					</div>
			</div>
