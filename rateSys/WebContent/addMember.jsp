					...
					$.post("AddMemberServlet/addData", {'pid' : pid,'pspid': pspid, 'pspgid': pspgid, 'isleader': isleader}, function(data) {
						if(data.message == "添加成功") {
					    	...
					    	var groupName = $("#groupName").val();
				    		 ...
				    		 var i,j;
				    		 var flag = 0; // 涉及到项目名、专家组名是否需要输出
				    		 for(i = 1; i <= trLength; i++){
				    			 j = i;
				    			 // 找到了当前项目行
				    			 if(pname == $("tr:eq(" + i + ")").find("td").eq(0).text()) {
				    				 flag = 1;
				    				 for(; j <= trLength; j++) {
				    					 // 找到了当前专家组行
				    					 if(groupName == $("tr:eq(" + j + ")").find("td").eq(1).text()) {
				    						 flag = 2;
				    						 break;
				    					 }
				    					 if($("tr:eq(" + (j+1) + ")").find("td").eq(0).text() !== "") {
				    						 break;
				    					 }
				    				 }
				    				 break;
							...
							if(j == trLength) j--; // 找到行尾都没有一条匹配记录，则插入行尾
							var trs = "<tr onclick='trClick(this);'>";
							//下面两个可以考虑忽略
							if(flag != 0) pname = "";
			    		 	trs += "<td class='t1'>" + pname + "</td>";
			    		 	if(flag != 0 && flag != 1) groupName = "";
				    		 trs += "<td class='t0'>" + groupName + "</td>";
				    		 trs += "<td class='t2'>" + memberName + "</td>";
				    		 
				    		 if(isleader == pspid)
				    		 	trs += "<td class='t4'><input type='radio' value='" + pspgid + "' name='" + pspgid + "' checked='checked' onclick='changeLeader(this)'><span></span></td>";
				    		 else trs += "<td class='t4'><input type='radio' value='" + pspgid + "' name='" + pspgid + "' onclick='changeLeader(this)'><span></span></td>";
				    		 
				    		 trs += "<td class='t9'><input type='button' value='删除' onclick='delData(this)'></td>";
		
				    		 trs += "<td style='display:none' >" + pspid + "</td>";
				    		 trs += "<td style='display:none' >" + pspgid + "</td>";
				    		 trs += "<td style='display:none' >" + groupName + "</td>";
				    		 trs += "<td style='display:none' >" + pname + "</td>";
				    		 trs += "</tr>";
				    		$("tr:eq("+ j + ")").after(trs);
						}
						else {
...
// 		    	 }
		    	 ...
		    	 for(var i = 0; i < tableJson.group.length; i++) {
		    		var flag = true;
		    		// 保存可以作为输出的项目id，在输出时，只有专家组的pid在map内，才可以输出该专家组、专家组成员 
...
//  		实现模糊查询功能 --> 修改 初始获取，然后中间遍历  
			...
			$(document).ready(function(){
				...
				$("#memberName").click(function(){ //点击时，则显示所有专家 
					var val = $(this).val();
					flage = true;
					var lis = "";
					for(var i = 0; i < allMember.length; i++) {
				...
				$("#groupName").click(function(){ //点击时，则显示所有专家 
					var val = $(this).val();
						...
						var first = false; // 该标记用来处理联动，即项目改变后，专家组名称跟着显示该项目下的第一个专家组。  
						for(var i = 0; i < groups.length; i++) {
							if(groups[i].pid == pid || pid == "") {
								lis += "<li onclick='mayGroupClick(this)'>" + groups[i].spgname +"</li>";
								// 项目改变后，专家组名称跟着显示该项目下的第一个专家组。  
								if(first == false) {
									first = true;
									$("#groupName").val(groups[i].spgname);
								}										
							}
						}
						$("#mayGroup").css("display","block");
...
<!-- 					<a href="changeSubPin.jsp"><li><img src="images/left2.png">下级密码</li></a> -->
					...
					<a href="addMember.jsp"><li class="now"><img src="images/left6-s.png">添加专家组成员</li></a>
...
<!-- 							<li>123</li><li>123</li><li>123</li> -->
					...
					<span style='margin-left: 6px;    width: 15rem;'>专家组：</span>
						<input type='text' id='groupName' name='groupName' autocomplete="off" style='width: 171px;padding-right: 18px;background: #f8f8f8 url(images/selectArr.png) no-repeat 178px center;'> 
						<ol id='mayGroup' class='leftOl'>
<!-- 							<li>123</li><li>123</li><li>123</li> -->
						</ol>
					<span style='margin-left: 4px;'>专家组成员：</span>
						<input type='text' id='memberName' name='memberName' autocomplete="off" style='width: 135px;padding-right: 18px;background: #f8f8f8 url(images/selectArr.png) no-repeat 144px center;'>
						<ol id='mayName' class='rightOl'>
<!-- 							<li>123</li><li>123</li><li>123</li> -->
						</ol>
