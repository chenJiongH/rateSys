...
// 					$("#group option:eq(0)").prop("selected", true);
					...
					if(data.mana.tid.charAt(0) == 'D') {
						console.log("here " + data.mana.tid);
			...
			var changePageFlag = false;
				...
				function dataShow() {
					...
					$(".page").html("");
			    	 ...
			    	 $(".page").append(lis);
		    		 console.log("here" + changePageFlag + " : " + changeGroup + " : " + initFlush);
			    	 
		    		 if(changePageFlag == true) {//点击分页，改变左表格。不是点击分页，改变右表格
			    		 //获取并生成当前页码左表格显示的数据 
			    		 dataShowLeft(); 
		    			 changePageFlag = false;
		    			 if(changeGroup == false && initFlush == false) //此时显示页面不是由于切换分组或刷新页面引起，不刷新右table 
		    			 	return ;
		    		 }
					initFlush = false;
		    		 console.log("here2");
		...
		<div class="main">
			...
			<div class="left">
				...
				<ul>
					...
					<a href="DexDis.jsp"><li><img src="images/left8.png">组内专家、指标管理</li></a>
					<br>
			...
			<div class="right">
				...
				<form class="center">
					...
					<span class="s2">市：</span>
						<select style="background-color: rgb(228, 226, 226);" name='city' id='city' class="sele2"></select>
					</br>
					<span class="s3">县：</span>
						<select style="background-color: rgb(228, 226, 226);" name='dist' id='dist' class="sele3"></select>
				</form>
