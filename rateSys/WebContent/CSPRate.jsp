...
<html>
	<head><meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
		<meta charset="utf-8">
		<title>市专家评分</title>
		<link href="css/CSPRate.css" rel="stylesheet"/>
		<style>
			.titleShow {
				max-width: 45rem;
				position: absolute;
		...
		function showProcess(pData) {
			...
			while(data.process.indexOf('-') != -1) {
				...
				if(rank == "校") {
					...
					$("div[class=ratemessage]").append('<label style="margin-right: 56px;">县（市、区）级</label>');
				} else if(rank == "市") {
					$("div[class=ratemessage]").append('<label style="margin-right: 91px;">设区市级</label>');
				} else if(rank == "省"){
					$("div[class=ratemessage]").append('<label style="margin-right: 92px;">省级</label>');
				} else if(rank == "抽") { 
			...
			$(function() {
				...
				var bottomBarTop = (top + 757) + "px";
			 	$(".bottom-bar").css({
						"top": bottomBarTop
				});
			})
			    //查找页面待显示内容，即当前校级管理员指标内容
				$.post("CSPRateServlet/findPr", function(data) {
					...
					$.post("CSPRateServlet/findCByPid", {'pid' : $("#Pr").val()}, function(data) {
						console.log(data);
						if(data.message != undefined) {
							$("#message").html("<img src='images/warn.png' class='messageWarn'>" + data.message);
							messageDisplay();
...
// 						构造所有的sidcid键值对，用空间换取时间、减少提交时判断数
						var scoreName = "cityScore", explainName = "cityExplain";
...
// 						构造所有的sidcid键值对，用空间换取时间、减少提交时判断数
			...
			function setReadonly() {
				...
				$(":input[value='查看']").removeAttr("onclick");
				$(":input[value='下载']").removeAttr("onclick");
			...
			function dataShow(data) {
				...
				for(var i = 0; i < data.c.length && start != 2; i ++) {
					if(curpage == data.c[i].sid) {
						...
						if(data.c[i].isexplain == 'TRUE' || data.c[i].isexplain== 'true') {/*  是否需要说明，无需说明则设置阈值为负无穷大  */
							sidcidExplain[data.c[i].sid + "" + data.c[i].cid] = true;
							if((data.c[i].cityScore != null && data.c[i].cityScore !== "")|| formData[data.c[i].sid + "" + data.c[i].cid] != undefined) {
								...
								if(formData[data.c[i].sid + "" + data.c[i].cid] == undefined) {
									...
									formData[data.c[i].sid + "" + data.c[i].cid] = data.c[i].cityScore;
									// id=sid+cid name=阈值 class=分段值  sidcidExplain
										
									trs += "<td class='t7 widChan' title='分段值:" + data.c[i].segscore +"' ><input segscore='" + data.c[i].segscore +"'  type='text' id='" + data.c[i].sid + "" + data.c[i].cid +"' name='" + data.c[i].threshhold +"' class='TRUE' onblur='grade(this)' value='" + data.c[i].cityScore + "'></td>";
								} else {		
									if(formData[data.c[i].sid + "" + data.c[i].cid] >= data.c[i].threshhold)
										delete sidcidExplain[data.c[i].sid + "" + data.c[i].cid];
									trs += "<td class='t7 widChan' title='分段值:" + data.c[i].segscore +"' ><input segscore='" + data.c[i].segscore +"'  type='text' id='" + data.c[i].sid + "" + data.c[i].cid +"' name='" + data.c[i].threshhold +"'  class='TRUE' onblur='grade(this)' value='" + formData[data.c[i].sid + "" + data.c[i].cid] + "'></td>";
								}
							}				
							else {
								trs += "<td class='t7 widChan' title='分段值:" + data.c[i].segscore +"' ><input segscore='" + data.c[i].segscore +"'  type='text' id='" + data.c[i].sid + "" + data.c[i].cid +"' name='" + data.c[i].threshhold +"'  class='TRUE' onblur='grade(this)'></td>";
							}
						}
						else {			
							if((data.c[i].cityScore != null && data.c[i].cityScore !== "")|| formData[data.c[i].sid + "" + data.c[i].cid] != undefined) {
								...
								if(formData[data.c[i].sid + "" + data.c[i].cid] == undefined) {
									formData[data.c[i].sid + "" + data.c[i].cid] = data.c[i].cityScore;
									trs += "<td class='t7 widChan' title='分段值:" + data.c[i].segscore +"' ><input  segscore='" + data.c[i].segscore +"' type='text' id='" + data.c[i].sid + "" + data.c[i].cid +"' name=''  onblur='grade(this)' value='" + data.c[i].cityScore + "'></td>";
								} else {
									trs += "<td class='t7 widChan' title='分段值:" + data.c[i].segscore +"' ><input  segscore='" + data.c[i].segscore +"' type='text' id='" + data.c[i].sid + "" + data.c[i].cid +"' name=''  onblur='grade(this)' value='" + formData[data.c[i].sid + "" + data.c[i].cid] + "'></td>";
								}
							} else {
								trs += "<td class='t7 widChan' title='分段值:" + data.c[i].segscore +"' ><input segscore='" + data.c[i].segscore +"'  type='text' id='" + data.c[i].sid + "" + data.c[i].cid +"' name=''  onblur='grade(this)'></td>";
							}
						}
						if(data.c[i].cityExplain != null && data.c[i].cityExplain !== "" || formData["0" + data.c[i].sid + data.c[i].cid] != undefined) {
							delete sidcidExplain[data.c[i].sid + "" + data.c[i].cid];
							if(formData["0" + data.c[i].sid + data.c[i].cid] == undefined) {
								formData["0" + data.c[i].sid + data.c[i].cid] = data.c[i].cityExplain;
								trs += "<td class='t8' title='"+data.c[i].cityExplain+"'><input maxlength='300' type='text' class='0" + data.c[i].sid + "" + data.c[i].cid + "' onblur='explain(this)' value='" + data.c[i].cityExplain + "'></td>";
							} else {
						...
						else trs += "<td class='t8'><input maxlength='300' type='text' class='0" + data.c[i].sid + "" + data.c[i].cid + "' onblur='explain(this)'></td>";
						trs += "</tr>";
					} else if(start == 1) {
						start = 2;
					}
				...
				// 刷新页面显示到自己的评分位置
				$(".bottom-bar span").css({"left": "345px"});
				var nowLeft = "456px";
				$("table").css({"left": "-456px"})
...
// 				获取小数位数, 将有可能为小数的分值段转化为整数，再利用取余运算符
			...
			function explain(e) {
				...
				if($(e).val() != "") {
					...
					// 分数小于阈值，且需要说明 
					var scoreInput = $($(e).parent().parent().find("td:eq(9)").children("input").get(0));
			...
			function rate() {
		      ...
		      zdconfirm('系统确认框',"提交后，评分信息将不可修改、无法回退。请确定是否提交？",function(r){ //项目删除确定框 
	    	     if(r) {
					formData.pid = $("#Pr").val();
					$.post("CSPRateServlet/rate", {'c' : JSON.stringify(formData)}, function(data) {
...
<!-- 				用于下载总评或附件 -->
				...
				<form id='form1' method='post' action='CSPRateServlet/rate' enctype="multipart/form-data">
					<span class="breakH" style='top: 145px;margin-left:-100px;'></span>
					<span class="breakV" style='top: 105px;margin-left: -100px;'></span>
					...
					<div id='editorBox' style='height: 76rem;margin-bottom: 2.9rem;z-index: 99999;width: 93rem;background-color: rgb(249, 249, 249);display: none;position: absolute;top: -84px;left: -26%;'>
			<!-- 		    保存或取消按钮 -->
			...
			<!-- 			编辑框 -->
					...
					<span style="width: 96px;position:absolute;right: 95%;top: 55%;margin-top:-0.6rem;	">项目名称：</span>
				...
				<div class="pagediv">
					...
					<table>
  						<tr id='row1'>
							<th colspan='3' style='background-color: rgb(236, 239, 253);z-index:3000;'></th>
					...
					<!-- 自定义滚动条 -->
					<div class="bottom-bar" style="left: 508px;top: 757px;">
		                <span></span>
		            </div>
					</div>
			</div>
...
(function($) {
    ...
    var GenerateCss = function () {
        ...
        $("#mb_msg").css({ padding: '2rem 0px 2rem 5px', lineHeight: '2rem',
